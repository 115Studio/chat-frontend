...
export const useChatMessagesStore = (id: string) => defineStore('channel-id-' + id, {
  state: () => ({
    messages: [] as Message[],
      ...
      if (index !== -1) {
        ...
        if (unprocessedStages.length > 0) {
          ...
          for (const stage of unprocessedStages) {
            if (message.updatedAt && message.updatedAt < stage.ts) this.updateMessagePartial(message.id, stage.stage, stage.ts)
          }
        }
      }
    },
